...
// ====================== 查询小猫列表 ======================
router.get('/cats', authenticateToken, authorize(PERMISSIONS.PET_READ), async (req, res) => {
    try {
...
// ====================== 新增小猫 ======================
router.post('/cats', authenticateToken, authorize(PERMISSIONS.PET_WRITE), validate([
...
// ====================== 修改小猫 ======================
router.put('/cats/:id', authenticateToken, authorize(PERMISSIONS.PET_WRITE), validate([
    body('name').trim().notEmpty().withMessage('名称不能为空'),
    body('age').notEmpty().withMessage('年龄不能为空'),
    body('breed').trim().notEmpty().withMessage('品种不能为空'),
    body('healthStatus').trim().notEmpty().withMessage('健康状态不能为空'),
...
// ====================== 删除小猫 ======================
router.delete('/cats/:id', authenticateToken, authorize(PERMISSIONS.PET_WRITE), async (req, res) => {
    try {
        const { id } = req.params;
        if (isNaN(id)) return sendResponse(res, false, null, 'ID必须是数字！', 400);
